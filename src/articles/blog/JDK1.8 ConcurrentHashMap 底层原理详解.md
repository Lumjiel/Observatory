...
- **Node**：基础链表节点，存储 `key/val/hash/next`，`val` 和 `next` 都用 `volatile` 修饰，保证并发可见性。
    
- **TreeBin**：红黑树的**包装节点**，不直接存数据，只维护红黑树的根与平衡，链表长度≥8 且数组≥64 时触发转换。
    
- **ForwardingNode**：扩容专用标记节点，`hash = MOVED(-1)`，占位在旧数组桶中，引导其他线程**协助扩容**，而非阻塞等待。
...
5. **三大分支处理**
    ...
    2. 桶为 `ForwardingNode`：当前正在扩容，线程**协助扩容**。
        
    3. 桶有数据：`synchronized` 锁定桶头节点，链表 / 红黑树插入。
...
- **多线程协同扩容**：扩容时其他线程不阻塞，直接 `helpTransfer()` 协助迁移，大幅提速。
    
- **纯无锁读**：get 全程不加锁，读多写少场景并发能力拉满。
