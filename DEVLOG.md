...
## 2026-05-16 — 修复 GitHub Actions 构建 + 完善文档

### 问题
每次推送到 GitHub，workflow 都报 `Process completed with exit code 1`。构建失败的原因是多个脚本文件缺失。

### 根因
1. **脚本文件缺失**：`build-js.mjs`、`article-scanner.mjs` 不存在，但 `package.json` 的 `build` 命令引用了它们
2. **工具模块缺失**：`categories.mjs`、`slug.mjs`、`reading-time.mjs` 不存在，但 `eleventy.config.js` 和 `article-service.mjs` 都 import 了它们
3. **路径配置错误**：`paths.mjs` 里写的是 `content/articles`，但实际文章在 `src/articles/`
4. **workflow 引用不存在的脚本**：`parse-git`、`gen-summary` 脚本不存在

### 改动

**新建 5 个文件：**
- `scripts/utils/categories.mjs` — 定义 blog/essays/projects/tutorials 四个分类
- `scripts/utils/slug.mjs` — 标题转 URL slug
- `scripts/utils/reading-time.mjs` — 中英文混排阅读时间计算
- `scripts/build-js.mjs` — esbuild 打包 JS（app.js → bundle.js，admin-panel.js → admin-panel.bundle.js）
- `scripts/article-scanner.mjs` — 扫描文章目录生成索引

**修改 4 个文件：**
- `scripts/utils/paths.mjs` — `content/` → `src/`（路径指向真实目录）
- `eleventy.config.js` — 图片复制路径 `content/images` → `src/img`
- `package.json` — `build` 和 `build:prod` 脚本补全 `build-js.mjs` 和 `article-scanner.mjs`
- `.github/workflows/build.yml` — 去掉不存在的 `parse-git`/`gen-summary` 步骤；deploy 改为仅手动触发时执行

**文档：**
- `README.md` — 全面更新：修正路径、补充缺失脚本、新增构建流程图、新增 GitHub Actions 和 GitHub Pages 部署章节

### 验证
本地 `npm run build` 通过，退出码 0，零错误零警告，生成 78 个文件。
